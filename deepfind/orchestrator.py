...
PLAN_PROMPT = (
    ...
    "web_search, and reserve Xiaohongshu, X/Twitter, Bilibili, YouTube, and BOSS Zhipin-specific tasks for the "
    "matching platform tools. Split it into {n} distinct research tasks. JSON array only."
)
WORKER_PROMPT = (
    ...
    "the Xiaohongshu, X/Twitter, Bilibili, YouTube, and BOSS Zhipin-specific tools for those platforms. Use "
    "boss_search for job searches, boss_detail when you need one posting's full description, boss_chatlist when "
    "you need existing BOSS chat threads or uid values, and boss_send when you need to send a follow-up message "
    "such as asking which company a hidden-company posting belongs to. Prefer boss_detail before messaging because "
    "it may already reveal the company. If the task mentions Bilibili video/audio, call bili_transcribe with the "
    "URL or BVID before summarizing. If the task mentions YouTube video/audio, call youtube_transcribe with the "
    'URL or video ID before summarizing. If the latest user request asks for an image, do not call gen_img unless '
    'the assigned task explicitly asks you to produce the final image asset. If the latest user request asks for '
    'slides, do not call gen_slides unless the assigned task explicitly asks you to produce the final slide asset. '
...
LEAD_PROMPT = (
    ...
    "Bilibili, YouTube, and BOSS Zhipin job tasks. For BOSS workflows, use boss_search/boss_detail first, then "
    "boss_chatlist to find the right conversation uid and boss_send for follow-up questions when needed. If the "
    "latest user request asks for an image, call gen_img exactly once with a concrete prompt, then mention the "
    "saved image path in the final answer. If the latest user request asks for slides, call gen_slides exactly once "
    "with a concrete prompt and slide_count, then mention the saved html path in the final answer. Do not claim an "
    "asset exists unless the corresponding tool succeeds."
)
